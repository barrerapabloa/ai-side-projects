...
export function BookingHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/[0.05] bg-[#07080a]/92 backdrop-blur-md">
      <div className="mx-auto flex max-w-5xl flex-col gap-3 px-4 py-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {search?.origin ? (
            <p className="max-w-[min(100%,22rem)] truncate text-right text-[13px] text-zinc-400 sm:max-w-none">
              ...
              {search.tripType === "round-trip" && search.returnDate ? (
                <>
                  <span className="text-zinc-600"> · </span>
                  ret {formatTripDate(search.returnDate)}
                </>
              ) : null}
              <span className="text-zinc-600"> · </span>
        ...
        {selectedFlight ? (
          <p className="text-[12px] text-zinc-500">
            Flight NMB {selectedFlight.flightNumber} · departs {selectedFlight.departLabel}{" "}
